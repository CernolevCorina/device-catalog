...
const Filters = () => {
	...
	return <form className={'flex flex-col sm:flex-row gap-2'} onSubmit={handleSubmit} onReset={handleReset}>
		<PriceFilter onChange={handleFilterChange} />
		<BrandFilter onChange={handleFilterChange} />
		<SortFilter onChange={handleFilterChange} />
		<div className={'flex gap-2'}>
			<button type={'submit'} className="rounded bg-primary px-4 py-2 text-foreground cursor-pointer">Apply</button>
			<button type={'reset'} className="rounded bg-dark px-4 py-2 text-foreground cursor-pointer">Reset</button>
		</div>
	</form>
};
